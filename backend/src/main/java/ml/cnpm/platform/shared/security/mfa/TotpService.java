...
/**
 * TOTP natif (RFC 6238 : HMAC-SHA1, 6 chiffres, période de 30 s).
 *
 * <p>Refonte du second facteur « façon SafeX 360 » (AUTH-DEC-020) : l'application détient
 * et vérifie elle-même le TOTP, sans fournisseur d'identité externe. Implémentation
 * autonome (aucune dépendance ajoutée), portée du {@code TotpService} de SafeX et validée
 * contre les vecteurs de test officiels de la RFC 6238.
 */
@Service
public class TotpService {
    ...
    /** Secret aléatoire (20 octets → 32 caractères Base32). */
    ...
    /** Retourne le pas valide, ou -1. Une tolérance de ±30 secondes est acceptée. */
    ...
    static byte[] decodeBase32(String encoded) {
        if (encoded == null || encoded.isBlank()) {
            throw new IllegalArgumentException("TOTP_SECRET_INVALID");
        }
