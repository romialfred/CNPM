...
async function evaluate(mode: CnpmDataMode, identity: SessionGateway['identity']) {
  ...
  const result = TestBed.runInInjectionContext(() =>
    bankStatementImportGuard({} as never, {} as never),
  );
  return isObservable(result) ? firstValueFrom(result) : await Promise.resolve(result);
}

describe('bankStatementImportGuard', () => {
  it('autorise le banc local en mode démo', async () => {
    await expect(evaluate('demo', of(IDENTITY))).resolves.toBe(true);
  });

  it('exige PAYMENT.RECORD en mode HTTP', async () => {
    await expect(
      evaluate('http', of({ ...IDENTITY, permissions: ['PAYMENT.RECORD'] })),
    ).resolves.toBe(true);
    const denied = await evaluate('http', of(IDENTITY));
    expect(TestBed.inject(Router).serializeUrl(denied as UrlTree)).toBe(
