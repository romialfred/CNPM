...
describe('MemberHomePage', () => {
  ...
  it('affiche une situation financière cohérente', async () => {
    await settle();

    const text = host.textContent ?? '';
    const normalizedText = text.replaceAll(/\s/g, ' ');
    expect(text).toContain('Sahel Agro SA');
    expect(text).toContain('CNPM-2026-0142');
  ...
  it('personnalise l’accueil sans bandeau structurel', async () => {
    await settle();

    expect(host.querySelector('h1')?.textContent).toContain('Bienvenue, Awa');
    expect(host.querySelector('.member-home__demo-note')).toBeNull();
