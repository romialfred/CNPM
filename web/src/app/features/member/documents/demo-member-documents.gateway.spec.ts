...
describe('DemoMemberDocumentsGateway — MP-012', () => {
  it('filtre, trie et pagine un catalogue déterministe', async () => {
    ...
    expect(page.items[0]?.reference).toBe('COGEF-DOC-0001');
    ...
    expect(filtered.items.map((item) => item.reference)).toEqual(['COGEF-DOC-0003']);

    const secondPage = await firstValueFrom(
      gateway.list({ ...DEFAULT_QUERY, sort: 'reference', direction: 'asc', page: 2 }),
    );
    expect(secondPage.items.map((item) => item.reference)).toEqual(['COGEF-DOC-0006']);
  });
  ...
  it('ne sert que des métadonnées publiques, sans contenu ni attribut GED interne', async () => {
    ...
    expect(document.reference).toMatch(/^COGEF-DOC-/);
