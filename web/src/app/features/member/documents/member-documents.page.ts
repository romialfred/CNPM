...
import {
  ...
  memberDocumentStatusTone,
  memberDocumentTypeLabel,
} from './member-document-presenter';
import {
  MEMBER_DOCUMENTS_GATEWAY,
  type MemberDocumentMetadata,
  type MemberDocumentQuery,
  type MemberDocumentSort,
  type MemberDocumentStatus,
  type MemberDocumentType,
} from './member-documents-gateway';

const PAGE_SIZES = [5, 10, 20] as const;
const STATUSES: readonly MemberDocumentStatus[] = ['CATALOGUED', 'PROCESSING', 'EXPIRED'];
const TYPES: readonly MemberDocumentType[] = ['ATTESTATION', 'MEMBER_CARD'];
const SORTS: readonly MemberDocumentSort[] = ['metadataRecordedOn', 'reference', 'title'];
...
@Component({
  ...
  templateUrl: './member-documents.page.html',
  styleUrl: './member-documents.page.scss',
})
export class MemberDocumentsPage {
  private readonly gateway = inject(MEMBER_DOCUMENTS_GATEWAY);
  ...
  protected readonly columns: readonly DataTableColumn[] = [
    ...
    { key: 'metadataRecordedOn', label: 'Métadonnée enregistrée' },
  ...
  protected readonly type = computed(() => known(this.params().get('type'), TYPES));
  protected readonly status = computed(() => known(this.params().get('status'), STATUSES));
  protected readonly sort = computed<MemberDocumentSort>(
    () => known(this.params().get('sort'), SORTS) ?? 'metadataRecordedOn',
  );
  protected readonly direction = computed<'asc' | 'desc'>(() =>
    this.params().get('order') === 'asc' ? 'asc' : 'desc',
  );
  ...
  private readonly query = computed<MemberDocumentQuery>(() => ({
    search: this.search(),
  ...
  protected readonly documents = computed(() => this.data()?.items ?? []);
  protected readonly totalItems = computed(() => this.data()?.totalElements ?? 0);
  protected readonly hasFilters = computed(() =>
    Boolean(this.search() || this.type() || this.status()),
  );
  protected readonly chips = computed<readonly FilterChip[]>(() => {
    const chips: FilterChip[] = [];
    if (this.search()) chips.push({ key: 'q', label: `Recherche : ${this.search()}` });
    if (this.type()) {
      chips.push({ key: 'type', label: `Type : ${memberDocumentTypeLabel(this.type()!)}` });
    }
    if (this.status()) {
      chips.push({
        key: 'status',
        label: `Statut : ${memberDocumentStatusLabel(this.status()!)}`,
      });
    }
    return chips;
  });
  protected readonly state = computed<DocumentListState>(() => {
    const result = this.result();
  ...
  protected readonly rowKey = (document: MemberDocumentMetadata): string => document.id;
  protected readonly statusLabel = memberDocumentStatusLabel;
  protected readonly statusTone = memberDocumentStatusTone;
  protected readonly typeLabel = memberDocumentTypeLabel;
  ...
  protected applyFilters(): void {
    ...
    this.patch({
      ...
      type: known(values.type, TYPES),
      status: known(values.status, STATUSES),
      sort: known(sort ?? null, SORTS) ?? 'metadataRecordedOn',
  ...
  protected resetFilters(): void {
    this.restoreResultsFocus.set(true);
    this.patch({ q: null, type: null, status: null, page: 1 });
  }
